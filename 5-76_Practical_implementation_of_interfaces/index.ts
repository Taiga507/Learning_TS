...
class SingleFileTransfer implements ITransfer, TransferError {
	...
	constructor(status: TransferStatus, path: string, data: string[], message: ErrorMessages) {
	...
	makeError = (status: string, error: string): string => {
		return `Status: ${status}, error message: ${error}`
	};
}

const transfer = new SingleFileTransfer(TransferStatus.Completed, 'path', ['12.05.2024'], ErrorMessages.NotEnoughSpace);
console.log(transfer.start());
console.log(transfer.checkTransferStatus());
console.log(transfer.stop('Test'));
console.log(transfer.makeError(TransferStatus.Pending, ErrorMessages.NotEnoughSpace));